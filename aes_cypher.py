...
def shift_rows(state: [[int]]):
  ...
  state[0][2], state[1][2], state[2][2], state[3][2] = state[2][2], state[3][
      2], state[0][2], state[1][2]
  state[0][3], state[1][3], state[2][3], state[3][3] = state[3][3], state[0][
      3], state[1][3], state[2][3]
...
def inv_shift_rows(state: [[int]]) -> [[int]]:
  state[1][1], state[2][1], state[3][1], state[0][1] = state[0][1], state[1][
      1], state[2][1], state[3][1]
  state[2][2], state[3][2], state[0][2], state[1][2] = state[0][2], state[1][
      2], state[2][2], state[3][2]
  state[3][3], state[0][3], state[1][3], state[2][3] = state[0][3], state[1][
...
if __name__ == "__main__":
  ...
  while True: 
    try: 
        nr = int(input("Digite o número de rodadas (0 a 10): ")) 
        if not 0 <= nr <= 10: 
            raise ValueError("Número fora do range permitido!") 
    except ValueError as e: 
        print("Valor inválido:", e) 
    else: 
        break
  plaintext = bytearray.fromhex('014BAF2278A69D331D5180103643E99A')
  key = bytearray.fromhex('E8E9EAEBEDEEEFF0F2F3F4F5F7F8F9FA')
  expected_ciphertext = bytearray.fromhex('6743C3D1519AB4F2CD9A78AB09A511BD')
  ...
  if ciphertext == expected_ciphertext and recovered_plaintext == plaintext:
    print("Cifragem Correta!")
  else:
    print("Cifragem Incorreta!")
